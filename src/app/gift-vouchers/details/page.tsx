...
import { Search, Gift, Calendar, TrendingUp, Eye, Plus, Edit, Trash2, X } from 'lucide-react';
...
export default function GiftVoucherDetailsPage() {
  ...
  const [editingGiftVoucher, setEditingGiftVoucher] = useState<GiftVoucher | null>(null);
  ...
  const fetchGiftVouchers = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter out deleted gift vouchers - only show active ones
        const activeGiftVouchers = response.data.data.filter((gv: GiftVoucher) => !gv.isDeleted);
        setGiftVouchers(activeGiftVouchers);
      } else {
  ...
  const handleDeleteClick = (id: string, giftVoucherCode: string) => {
    setDeleteDialog({
      isOpen: true,
      giftVoucherId: id,
      giftVoucherCode
  ...
  const handleDeleteConfirm = async () => {
    if (!deleteDialog.giftVoucherId) return;

    try {
      const response = await apiCall(`/admin/gift-vouchers/${deleteDialog.giftVoucherId}`, {
        method: 'DELETE'
      });
      ...
      if (response.data.success) {
        ...
        toast.error('Failed to delete gift voucher');
      }
    } catch (error) {
      toast.error('Failed to delete gift voucher');
    } finally {
  ...
  const handleEdit = (giftVoucher: GiftVoucher) => {
    setEditingGiftVoucher(giftVoucher);
    setFormData({
      code: giftVoucher.code,
      name: giftVoucher.name,
      description: giftVoucher.description || '',
      type: giftVoucher.type,
      value: giftVoucher.value.toString(),
      minimumAmount: giftVoucher.minimumAmount.toString(),
      maximumDiscount: giftVoucher.maximumDiscount?.toString() || '',
      usageLimit: giftVoucher.usageLimit?.toString() || '',
      validFrom: giftVoucher.validFrom ? new Date(giftVoucher.validFrom).toISOString().split('T')[0] : '',
      validUntil: giftVoucher.validUntil ? new Date(giftVoucher.validUntil).toISOString().split('T')[0] : '',
      isActive: giftVoucher.isActive,
      isOneTimeUse: giftVoucher.isOneTimeUse || false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.code.trim()) {
      newErrors.code = 'Gift voucher code is required';
    }

    if (!formData.name.trim()) {
      newErrors.name = 'Gift voucher name is required';
    }

    if (!formData.value.trim()) {
      newErrors.value = 'Gift voucher value is required';
    }
    ...
    try {
      ...
      if (editingGiftVoucher) {
        const response = await apiCall(`/admin/gift-vouchers/${editingGiftVoucher._id}`, {
          method: 'PUT',
          body: JSON.stringify(giftVoucherData)
        });

        if (response.data.success) {
          toast.success('Gift voucher updated successfully!');
          setShowAddModal(false);
          setEditingGiftVoucher(null);
          resetForm();
          fetchGiftVouchers();
        } else {
          toast.error('Failed to update gift voucher');
        }
      } else {
        const response = await apiCall('/admin/gift-vouchers', {
          method: 'POST',
          body: JSON.stringify(giftVoucherData)
        });
      ...
      toast.error('Failed to save gift voucher');
    }
  };

  const filteredGiftVouchers = giftVouchers.filter(voucher =>
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900 mt-4">Gift Voucher Details</h1>
            <p className="text-gray-600 mt-2">View all gift vouchers and their statistics</p>
          </div>
          <button
            onClick={() => {
              setEditingGiftVoucher(null);
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto relative">
              ...
              <h2 className="text-xl font-semibold text-gray-900 mb-4">
                {editingGiftVoucher ? 'Edit Gift Voucher' : 'Add Gift Voucher'}
              </h2>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center space-x-3 pt-4">
                  ...
                  >
                    {editingGiftVoucher ? 'Update Gift Voucher' : 'Create Gift Voucher'}
                  </button>
                  <button
                    type="button"
                    onClick={() => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            ...
            paginatedGiftVouchers.map((voucher) => (
              <div key={voucher._id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div className="relative">
                  {voucher.image && (
                    <img
                      src={voucher.image}
                  ...
                  <div className="absolute top-2 right-2 flex space-x-2">
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                      ...
                      {voucher.isActive && !voucher.isExpired ? 'Active' : 'Inactive'}
                    </span>
                    {voucher.isExpired && (
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <span className="font-mono font-bold text-lg text-indigo-700">{voucher.code}</span>
                  ...
                  <div className="text-xs text-gray-500 space-y-1">
                    <p>Min. Amount: ₹{voucher.minimumAmount}</p>
                    {voucher.maximumDiscount && <p>Max. Discount: ₹{voucher.maximumDiscount}</p>}
                    {voucher.usageLimit && (
                      <p>Usage: {voucher.usageCount || 0}/{voucher.usageLimit}</p>
                    )}
                    {/* Analytics Data */}
                    <div className="mt-3 pt-3 border-t space-y-2">
                      {voucher.usageCount !== undefined && (
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs font-semibold text-gray-900">{voucher.usageCount} times</span>
                        </div>
                      )}
                      {voucher.totalRevenue !== undefined && (
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-gray-600">Revenue:</span>
                          <span className="text-xs font-semibold text-green-600">₹{voucher.totalRevenue.toLocaleString()}</span>
                        </div>
                      )}
                      {voucher.totalDiscount !== undefined && (
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-gray-600">Discount Given:</span>
                          <span className="text-xs font-semibold text-blue-600">₹{voucher.totalDiscount.toLocaleString()}</span>
                        </div>
                      )}
                    </div>
                    <div className="mt-2 pt-2 border-t">
                      <p className="flex items-center space-x-1">
                        ...
                        <span>From: {formatDate(voucher.validFrom)}</span>
                      </p>
                      {voucher.validUntil && (
                        <p className="flex items-center space-x-1">
                          <Calendar className="w-3 h-3" />
                          <span>Until: {formatDate(voucher.validUntil)}</span>
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t mt-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      onClick={() => handleDeleteClick(voucher._id, voucher.code)}
                  ...
                  <Link
                    href={`/gift-vouchers/${voucher._id}/analytics`}
        ...
        <ConfirmDialog
          ...
          title="Delete Gift Voucher"
          message={`Are you sure you want to delete gift voucher "${deleteDialog.giftVoucherCode}"?`}
          confirmText="Delete"
          cancelText="Cancel"
          type="danger"
        />
